...
if(isset($_SESSION['ville']) AND ($_SESSION['ville']!="1") )
{
	...
	$sql_distance="SELECT ville_proxi_lat, ville_proxi_long FROM ville_proxi WHERE ville_proxi_nom=:ville_proxi_nom";
	...
	$sql_ile="SELECT liste_ile_id, liste_ile_latitude, liste_ile_longitude FROM liste_ile WHERE 1";
	...
	while($resultat_ile=fetch_object($exe_ile))
	{
		...
		$sql_update_distance="UPDATE `liste_ile` SET `liste_ile_distance` =:liste_ile_distance WHERE `liste_ile`.`liste_ile_id` =:liste_ile_id";
	...
	$sql="SELECT * FROM liste_ile WHERE 1 ORDER BY liste_ile_distance ASC";
	
}
if(isset($_SESSION['tri']) AND ($_SESSION['tri']!="1"))
{
	$sql="SELECT * FROM liste_ile WHERE 1 ".$_SESSION['tri'];
}
...
{
	$sql="SELECT * FROM liste_ile WHERE 1 ORDER BY liste_ile_nom ASC";
}
$exe=query($sql);
$sql2="SELECT DISTINCT ville_proxi_nom FROM ville_proxi WHERE 1 ORDER BY ville_proxi_nom ASC ";
$exe2=query($sql2);
